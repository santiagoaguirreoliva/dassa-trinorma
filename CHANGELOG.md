...
- **Primer acceso por invitación**: desde la vista de Empleados, cada persona tiene un **link de primer ingreso** (`employees.portal_invite_token`, migración 060). Con el link el empleado entra una vez, **crea su propio PIN** (4 dígitos, único global, rechaza triviales) y completa el onboarding. Botón **"Acceso"** en cada fila de la tabla de Empleados (columna Acciones) → modal con el link, copiar y regenerar (resetea PIN).
