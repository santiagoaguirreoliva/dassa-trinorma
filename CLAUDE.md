...
## Estado al cierre 2026-08-26 · Día 3 de BV — comité, indicadores y acta entregable
2 commits más (`348b9c6`, `5e7970a`) pusheados. Cierre de la auditoría de Bureau Veritas.
- **Acta del comité mixto**: la tarjeta y el detalle mostraban sólo el contador de asistentes — ahora listan **quiénes estuvieron** (el acta de un comité es nominal). La minuta de la reunión del 08/07 quedó redactada y la reunión pasó a `realizada`.
- 🆕 **Exportar el acta de Revisión por la Dirección** (`printActa` en `RevisionDireccion.tsx`, mismo patrón que `printPlanilla` de Capacitaciones): membrete con `/ds/logos/dassa-isotipo.png` + las tres normas + `docRef('F-TRI-05')`, entradas de desempeño con la tabla de los 16 objetivos y su última medición, análisis 9.3.2 (9001 + 14001/45001 en bloque aparte), decisiones 9.3.3 con salidas y acciones, y pie de firmas. **Un campo vacío se imprime como "Sin completar"** (el acta debe dejar ver qué falta) y los firmantes repetidos se deduplican por nombre. Verificado renderizando el HTML real con los datos del acta 2026 (Chrome headless + `--print-to-pdf`).
- **Objetivos (data-only)**: **16 visibles**, 13 con serie 2026. Se revivieron 4 del F-TRI-04 histórico de Drive (EBITDA con la serie real `Rentabilidad EERR` copiada del OBJ-02 oculto · Bienestar con baseline 2025 · Papel · Aysa) y se **reactivó Capacitaciones** — ocultarlo el 25/08 fue un error de diagnóstico: había asistencia real desde mayo (may 86,2% · jun 75,0% · ago 100%). **Julio no se cargó**: cargaron participantes sin marcar asistencia (15,4% irreal), mismo criterio que el forzoso feb-may.
- Gotcha del cierre: **dos sesiones concurrentes eligieron el mismo ID de handoff** y un `cat >` pisó el archivo de la otra. Al escribir en `historico-sesiones/`, verificar existencia justo antes y usar `set -o noclobber`.
- Pendientes: cómo cerró BV · avisar a Nixa que el docx de P-TRI-14 en Drive tiene el objeto copiado del de exportación · asistencia del 16/07 + participantes de las charlas de salud (destraban OBJ-2026-12 y 13) · IMPO 2025 del cuadro · OEA sin confirmar.
