...
router.post('/public/nc', async (req, res) => {
  const {
  ...
  try {
    ...
    // Insertar NC con finding_type nc_real y status abierto
    ...
    // Notificación in-app a todos los sgi_leader y master_admin
...
router.get('/', async (req, res) => {
  const { status, type, area, assigned } = req.query;
  const where = ['1=1'];
  const params = [];
  let i = 1;
...
// POST /api/findings — crear NC interna
...
router.patch('/:id/status', async (req, res) => {
  ...
  try {
    const { rows } = await query(
      `UPDATE findings SET status = $1 WHERE id = $2 RETURNING id, code, status`,
...
router.delete('/:id', requireRole('master_admin'), async (req, res) => {
  try {
