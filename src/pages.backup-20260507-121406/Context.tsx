...
const FODA_CONFIG = {
  ...
  oportunidad: { label: 'Oportunidades', color: 'bg-blue-500',    light: 'bg-blue-50 border-blue-200',      icon: TrendingUp, letter: 'O' },
...
const STATUS_COLORS: Record<string, string> = { planned: 'bg-slate-100 text-slate-700', in_progress: 'bg-blue-100 text-blue-700', completed: 'bg-emerald-100 text-emerald-700', cancelled: 'bg-red-100 text-red-700' };

const STAKEHOLDER_TYPES = ['interno', 'externo'];
const INFLUENCE_LEVELS = ['bajo', 'medio', 'alto', 'critico'];
const INFLUENCE_COLORS: Record<string, string> = { bajo: 'bg-slate-100 text-slate-700', medio: 'bg-blue-100 text-blue-700', alto: 'bg-amber-100 text-amber-700', critico: 'bg-red-100 text-red-700' };
      ...
      <Header
        ...
        actions={
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-blue-600 text-white rounded-lg text-[13px] font-semibold hover:bg-blue-700"
          >
        ...
        return (
          <div key={type} className={`border rounded-xl p-4 ${cfg.light}`}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="ml-auto text-xs font-medium text-slate-500">{items.length}</span>
            </div>
            {items.length === 0 && <p className="text-xs text-slate-400 italic">Sin elementos cargados</p>}
            <div className="space-y-2">
              {items.map((item) => (
                <div key={item.id} className="bg-white rounded-lg p-3 shadow-sm flex items-start gap-2 group">
                  <div className="flex-1 min-w-0">
                    <span className="text-[11px] font-semibold text-slate-400 uppercase">{item.category}</span>
                    <p className="text-[13px] text-slate-700">{item.description}</p>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => onEdit(item)} className="p-1 hover:bg-slate-100 rounded"><Pencil size={13} className="text-slate-400" /></button>
...
function StrategiesTable({ strategies, users, onEdit, onDelete }: { strategies: Strategy[]; users: User[]; onEdit: (s: Strategy) => void; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      {grouped.map((g) => (
        <div key={g.value}>
          <h3 className="text-sm font-bold text-slate-600 mb-2">{g.label} ({g.items.length})</h3>
          {g.items.length === 0 && <p className="text-xs text-slate-400 italic mb-3">Sin estrategias</p>}
          <div className="space-y-2">
            {g.items.map((s) => (
              <div key={s.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-center gap-3 group">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-semibold text-[13px] text-slate-700">{s.name}</span>
                  ...
                  {s.description && <p className="text-[12px] text-slate-500 line-clamp-2">{s.description}</p>}
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onEdit(s)} className="p-1 hover:bg-slate-100 rounded"><Pencil size={13} className="text-slate-400" /></button>
...
function StakeholdersTable({ stakeholders, onEdit, onDelete }: { stakeholders: Stakeholder[]; onEdit: (s: Stakeholder) => void; onDelete: (id: string) => void }) {
  ...
  const renderGroup = (label: string, items: Stakeholder[]) => (
    <div>
      <h3 className="text-sm font-bold text-slate-600 mb-2">{label} ({items.length})</h3>
      {items.length === 0 && <p className="text-xs text-slate-400 italic mb-3">Sin partes interesadas</p>}
      <div className="space-y-2">
        {items.map((s) => (
          <div key={s.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-center gap-3 group">
            ...
            <div className="flex-1 min-w-0">
              ...
              {s.needs_expectations && <p className="text-[12px] text-slate-500 mt-0.5 line-clamp-2">{s.needs_expectations}</p>}
...
function FodaModal({ initial, onClose, onSave, saving }: { initial?: FodaItem | null; onClose: () => void; onSave: (d: any) => void; saving: boolean }) {
  ...
  return (
    <ModalShell title={initial ? 'Editar Elemento FODA' : 'Nuevo Elemento FODA'} onClose={onClose}>
      <div className="space-y-4">
        <div>
          <label className="block text-[12px] font-semibold text-slate-500 mb-1">Tipo FODA</label>
        ...
        <div>
          <label className="block text-[12px] font-semibold text-slate-500 mb-1">Descripción</label>
        ...
        <button
          ...
          className="w-full py-2.5 bg-blue-600 text-white rounded-lg text-[13px] font-semibold hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-2"
        >
    ...
    <ModalShell title={initial ? 'Editar Estrategia' : 'Nueva Estrategia'} onClose={onClose}>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-[12px] font-semibold text-slate-500 mb-1">Tipo</label>
        ...
        <div>
          <label className="block text-[12px] font-semibold text-slate-500 mb-1">Nombre</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-[12px] font-semibold text-slate-500 mb-1">Responsable</label>
          ...
          <div>
            <label className="block text-[12px] font-semibold text-slate-500 mb-1">Plazo</label>
    ...
    <ModalShell title={initial ? 'Editar Parte Interesada' : 'Nueva Parte Interesada'} onClose={onClose}>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-[12px] font-semibold text-slate-500 mb-1">Influencia</label>
        ...
        <div>
          <label className="block text-[12px] font-semibold text-slate-500 mb-1">Necesidades y Expectativas</label>
