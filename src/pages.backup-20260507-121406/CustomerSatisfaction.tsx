    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-[15px] font-extrabold text-slate-900">Nueva Encuesta</h3>
          <button onClick={onClose}><X size={18} className="text-slate-400" /></button>
        </div>
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          <div>
            <label className="block text-[12px] font-semibold text-slate-500 mb-1">Título <span className="text-red-500">*</span></label>
            <input value={form.title} onChange={e => set('title', e.target.value)}
              placeholder="Ej: Encuesta Satisfacción Q1 2026" className="w-full border rounded-lg px-3 py-2 text-[13px]" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-semibold text-slate-500 mb-1">Tipo <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="block text-[12px] font-semibold text-slate-500 mb-1">Período</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-semibold text-slate-500 mb-1">Año</label>
            ...
            <div>
              <label className="block text-[12px] font-semibold text-slate-500 mb-1">Trimestre</label>
          ...
          <div>
            <label className="block text-[12px] font-semibold text-slate-500 mb-1">Fecha de cierre</label>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <p className="text-[13px] font-bold text-slate-800 mb-1">Indicador NPS</p>
              <p className="text-[11px] text-slate-400 mb-4">Promotores ({promoters}) − Detractores ({detractors}) = NPS {nps}</p>
              {totalResponses > 0 ? (
                <>
                  <div className="flex-1 h-6 bg-slate-100 rounded-full overflow-hidden flex mb-3">
                ...
                <p className="text-[12px] text-slate-400 text-center py-6">Sin respuestas NPS registradas aún</p>
              )}
            </div>
          </div>
        ) : (
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="text-left px-4 py-3 text-[11px] font-bold text-slate-500 uppercase">Encuesta</th>
                    <th className="text-left px-4 py-3 text-[11px] font-bold text-slate-500 uppercase w-24">Tipo</th>
                    <th className="text-center px-4 py-3 text-[11px] font-bold text-slate-500 uppercase w-20">Resp.</th>
                    <th className="text-left px-4 py-3 text-[11px] font-bold text-slate-500 uppercase w-24">Período</th>
                    <th className="text-center px-4 py-3 text-[11px] font-bold text-slate-500 uppercase w-20">Estado</th>
                    <th className="text-left px-4 py-3 text-[11px] font-bold text-slate-500 uppercase w-24">Cierre</th>
                    {isAdmin && <th className="text-right px-4 py-3 text-[11px] font-bold text-slate-500 uppercase w-20">Acc.</th>}
                  </tr>
                </thead>
                <tbody>
                  {surveys.map(s => (
                    <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3">
                        ...
                        {s.description && <p className="text-[11px] text-slate-400 truncate max-w-[250px]">{s.description}</p>}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center justify-center min-w-[28px] px-2 py-1 rounded-full bg-blue-50 text-blue-700 text-[11px] font-bold">
                          {s.response_count}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[11px] text-slate-500">{s.period || (s.year ? `${s.year} Q${s.quarter || '?'}` : '—')}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${s.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                          {s.is_active ? 'Activa' : 'Cerrada'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[11px] text-slate-500">{fmtDate(s.closes_at)}</span>
                      </td>
                      {isAdmin && (
