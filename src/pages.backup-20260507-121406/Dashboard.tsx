          ...
          <div className="space-y-4 md:space-y-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-5">
                ...
                <p className="text-[11px] text-slate-400 mb-3">Por nivel de IR (P×S)</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-5">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[13px] font-bold text-slate-800">Mis Tareas</p>
                  <button onClick={() => navigate('/tasks')}
                    className="text-[11px] text-blue-500 hover:underline flex items-center gap-0.5 touch-manipulation">
                  ...
                  {myTasks.slice(0, 6).map(t => {
                    ...
                    return (
                      <div key={t.id} className={[
                        ...
                        <span className="flex-1 min-w-0 font-medium text-slate-700 leading-snug line-clamp-2">
                          {t.title}
                        </span>
                        {t.finding_code && (
                    ...
                    {findings.slice(0, 4).map(f => {
                      ...
                      return (
                        <div key={f.id}
                          ...
                          <span className="flex-1 min-w-0 text-[11px] font-medium text-slate-700 line-clamp-1">{f.title}</span>
                          {sc && <Badge label={sc.label} variant={sc.variant} size="sm" />}
                          <span className={`text-[10px] font-bold flex-shrink-0 ${d > 15 ? 'text-red-500' : 'text-slate-400'}`}>{d}d</span>
                        </div>
                      );
                    })}
                    {findings.length === 0 && (
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-5">
                    ...
                    <div className="space-y-2">
                      {legal.slice(0, 3).map(l => (
                        <div key={l.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0 gap-3">
                          <div className="min-w-0 flex-1">
                            <p className="text-[12px] font-semibold text-slate-700 truncate">{l.title}</p>
