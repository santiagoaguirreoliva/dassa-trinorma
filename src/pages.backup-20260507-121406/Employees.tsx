...
const STATUS_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
  ...
  inactivo:  { label: 'Inactivo',   bg: 'bg-slate-100',   color: 'text-slate-500' },
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-[15px] font-extrabold text-slate-900">{isEdit ? 'Editar Empleado' : 'Nuevo Empleado'}</h3>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center gap-3 flex-wrap">
              ...
              <span className="ml-auto text-xs text-slate-400">{employees.length} empleados</span>
            </div>
                    ...
                    return (
                      <tr key={emp.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-3">
                          <code className="text-[10px] font-extrabold text-blue-700">{emp.employee_number}</code>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              {emp.position && <p className="text-[10px] text-slate-400">{emp.position}</p>}
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 hidden lg:table-cell">
                          <span className="text-[10px] text-slate-400">{fmtDate(emp.hire_date)}</span>
                        </td>
