    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-[15px] font-extrabold text-slate-900">{isEdit ? 'Editar Aspecto Ambiental' : 'Nuevo Aspecto Ambiental'}</h3>
        ...
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="bg-slate-50 rounded-lg p-3 flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500">Significancia (F × S × D)</span>
            <SigBadge sig={sig} />
          </div>
          <div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center gap-3 flex-wrap">
              ...
              <span className="ml-auto text-xs text-slate-400">{filtered.length} aspectos</span>
            </div>
                  ...
                  {filtered.map(a => (
                    <tr key={a.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors
                      ...
                      <td className="px-4 py-3 max-w-[200px]">
                        <p className="text-xs font-semibold text-slate-800 truncate">{a.aspect}</p>
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell max-w-[160px]">
                        <p className="text-[10px] text-slate-500 truncate">{a.activity || '—'}</p>
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell max-w-[160px]">
                        <p className="text-[10px] text-slate-500 truncate">{a.impact || '—'}</p>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="text-xs font-bold text-slate-700">{a.frequency}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="text-xs font-bold text-slate-700">{a.severity}</span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="text-xs font-bold text-slate-700">{a.detection}</span>
                      </td>
                      <td className="px-4 py-3">
                        <SigBadge sig={a.significance} />
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell max-w-[160px]">
                        <p className="text-[10px] text-slate-500 truncate">{a.control_measure || '—'}</p>
                      </td>
                      {isAdmin && (
                        <td className="px-4 py-3">
