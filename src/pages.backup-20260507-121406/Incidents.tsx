...
const STATUS_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
  ...
  investigacion: { label: 'Investigación', bg: 'bg-blue-100',     color: 'text-blue-700' },
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-[15px] font-extrabold text-slate-900">{isEdit ? 'Editar Incidente' : 'Nuevo Incidente / Accidente'}</h3>
            ...
            <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center gap-3 flex-wrap">
              <div className="relative min-w-[180px] max-w-xs">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)}
                  placeholder="Buscar..."
                  className="w-full pl-8 pr-3 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-blue-400" />
              </div>
              ...
              <span className="ml-auto text-xs text-slate-400">{incidents.length} registros</span>
            </div>
                    ...
                    return (
                      <tr key={inc.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors
                        ...
                        <td className="px-4 py-3">
                          <code className="text-[10px] font-extrabold text-blue-700">{inc.code}</code>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-[10px] text-slate-500">{fmtDate(inc.date)}</span>
                        </td>
                        ...
                        <td className="px-4 py-3 hidden lg:table-cell max-w-[180px]">
                          <p className="text-[10px] text-slate-500 truncate">{inc.corrective_action || '—'}</p>
                        </td>
                        {isAdmin && (
                          <td className="px-4 py-3">
