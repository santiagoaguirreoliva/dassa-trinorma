    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[13px] font-bold text-slate-800">{section.label}</h3>
            <p className="text-[10px] text-slate-400">{section.description}</p>
          </div>
        </div>
        {isAdmin && !editing && (
          <button onClick={() => setEditing(true)}
            className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-bold text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
      ...
      <div className="p-5">
        {editing ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 justify-end">
              ...
              <button onClick={() => mutation.mutate()}
                ...
                className="flex items-center gap-1.5 px-4 py-1.5 bg-blue-700 text-white font-bold text-xs rounded-lg hover:bg-blue-600 disabled:opacity-50">
          ...
          <div className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap min-h-[60px]">
            {content || <span className="text-slate-300 italic">Sin contenido definido. Haga clic en Editar para agregar.</span>}
          </div>
        )}
      </div>
    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-slate-600">Completitud del SGI</span>
                <span className="text-xs font-bold text-blue-600">{completeSections}/{totalSections} secciones</span>
              </div>
