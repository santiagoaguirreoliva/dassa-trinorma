...
function FTri36View({ training, participants }: { training: any; participants: Participant[] }) {
  return (
    <div className="bg-white border-2 border-slate-300 rounded-xl overflow-hidden text-sm">
      {/* Header membrete */}
      <div className="bg-slate-800 text-white px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-slate-400">DEPÓSITO AVELLANEDA SUR S.A.</p>
          </div>
        </div>
        ...
        <table className="w-full text-xs">
          <tbody>
            <tr className="border-b border-slate-200">
              <td className="px-4 py-2 font-bold bg-slate-50 w-36 border-r border-slate-200">Nombre del Curso:</td>
              <td className="px-4 py-2 flex-1">{training.title}</td>
              <td className="px-4 py-2 font-bold bg-slate-50 w-32 border-l border-r border-slate-200">Fecha del curso:</td>
              <td className="px-4 py-2 w-32">{new Date(training.scheduled_date).toLocaleDateString('es-AR')}</td>
            </tr>
            <tr className="border-b border-slate-200">
              <td className="px-4 py-2 font-bold bg-slate-50 border-r border-slate-200">Capacitador:</td>
              <td className="px-4 py-2">{training.instructor || '—'}</td>
              <td className="px-4 py-2 font-bold bg-slate-50 border-l border-r border-slate-200">Duración:</td>
              <td className="px-4 py-2">{training.duration_hours ? `${training.duration_hours} hs` : '—'}</td>
            </tr>
            <tr className="border-b border-slate-200">
              <td className="px-4 py-2 font-bold bg-slate-50 border-r border-slate-200" rowSpan={2}>Temario desarrollado:</td>
              <td className="px-4 py-2" rowSpan={2}>{training.description || '—'}</td>
              <td className="px-4 py-2 font-bold bg-slate-50 border-l border-r border-slate-200">Hora de inicio y cierre:</td>
              <td className="px-4 py-2">{new Date(training.scheduled_date).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })} hs</td>
            </tr>
            <tr className="border-b border-slate-200">
              <td className="px-4 py-2 font-bold bg-slate-50 border-l border-r border-slate-200">Firma del capacitador:</td>
              <td className="px-4 py-2 h-8"></td>
            </tr>
            <tr>
              <td className="px-4 py-2 font-bold bg-slate-50 border-r border-slate-200">Objetivo:</td>
      ...
      {/* Lista participantes */}
      <div>
        <div className="bg-slate-100 px-4 py-2 text-center font-extrabold text-xs border-b border-slate-200 uppercase tracking-widest">
            ...
            {Array.from({ length: Math.max(16, participants.length) }).map((_, i) => {
              ...
              return (
                <tr key={i} className="border-b border-slate-100">
                  ...
                  <td className="px-3 py-2 text-slate-500">{sector}</td>
      ...
      <div className="border-t-2 border-slate-300">
        <table className="w-full text-xs">
          <tbody>
            <tr className="border-b border-slate-200">
              <td className="px-4 py-3 font-bold bg-slate-50 border-r border-slate-200">
        ...
        <div className="sticky top-0 bg-white border-b border-slate-200 z-10 px-6 py-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <h2 className="text-[15px] font-extrabold text-slate-900 leading-snug">{data.title}</h2>
                ...
                return (
                  <button key={s} onClick={() => updateStatus.mutate(s)}
                    className={`px-2.5 py-1 rounded-full text-[10px] font-bold transition-colors
                      ${active ? `${sc2.bg} ${sc2.text}` : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}>
                    {sc2.label}
                  </button>
                );
              })}
        ...
        <div className="flex-1 p-6 space-y-4">
          ...
          {tab === 'info' && (
            <div className="space-y-4">
              ...
              {data.objective && <Field label="Objetivo"><p className="text-sm text-slate-700">{data.objective}</p></Field>}
              {data.legal_framework && <Field label="Marco legal"><p className="text-sm text-slate-700">{data.legal_framework}</p></Field>}
              {data.description && <Field label="Descripción"><p className="text-sm text-slate-700">{data.description}</p></Field>}
              {total > 0 && (
                <div className={`p-4 rounded-xl border ${attended === total ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-bold text-slate-700">Asistencia</p>
                    <p className="text-xs font-extrabold text-slate-800">{attended}/{total}</p>
                  </div>
              ...
              {total > 0 && (
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="font-semibold text-slate-600">Asistencia registrada</span>
                ...
                {(data.participants || []).map((p: Participant) => {
                  ...
                  return (
                    <div key={p.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors
                      ${p.attended ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
                      <button
                        onClick={() => toggleAttendance.mutate({ pid: p.id, attended: !p.attended })}
                        className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors
                          ${p.attended ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 hover:border-emerald-400'}`}>
                        {p.attended && <CheckCircle2 size={12} className="text-white" />}
                      </button>
                      {p.user_id
                        ? <Avatar name={name} size={28} />
                        : <div className="w-7 h-7 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 text-[10px] font-bold flex-shrink-0">EXT</div>
                      }
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-slate-800 truncate">{name}</p>
                        <p className="text-[10px] text-slate-400">{sector}{isExt ? ' · Externo' : ''}</p>
                      </div>
                      {isAdmin && (
                ...
                <div className="space-y-2 pt-2">
                  ...
                  {!showAddExt ? (
                    <button onClick={() => setShowAddExt(true)}
                      className="w-full py-2 border border-dashed border-slate-300 rounded-xl text-xs text-slate-400 hover:border-blue-400 hover:text-blue-500 transition-colors">
                      + Agregar participante externo
                    </button>
                  ) : (
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 space-y-2">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Participante externo</p>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setShowAddExt(false)} className="flex-1 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-500">Cancelar</button>
                        <button onClick={() => newExt.name && addParticipantExt.mutate()} disabled={!newExt.name}
                          className="flex-1 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-bold hover:bg-blue-700 disabled:opacity-50">Agregar</button>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {tab === 'evidencia' && (
            <div className="space-y-3">
              ...
              {(data.evidence || []).map((ev: Evidence) => (
                <div key={ev.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-200">
                  ...
                  {isAdmin && (
                    <button onClick={() => deleteEvidence.mutate(ev.id)} className="text-slate-300 hover:text-red-400 flex-shrink-0">
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
              ...
              {isAdmin && (
                <button onClick={() => fileRef.current?.click()} disabled={uploadEvidence.isPending}
                  className="w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-slate-300 rounded-xl text-sm text-slate-500 hover:border-blue-400 hover:bg-blue-50 transition-colors">
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-[15px] font-extrabold text-slate-900">Nueva Capacitación</h3>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center gap-2 pt-5">
              ...
              <label htmlFor="mandatory" className="text-sm font-semibold text-slate-700 cursor-pointer">
                Obligatoria
              </label>
            </div>
          </div>
    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="grid grid-cols-7">
        {Array.from({ length: adjustedFirst }).map((_, i) => (
          <div key={`e-${i}`} className="h-20 border-r border-b border-slate-100 bg-slate-50" />
        ))}
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <div key={day} className={`h-20 border-r border-b border-slate-100 p-1 relative ${isToday ? 'bg-blue-50' : ''}`}>
              ...
              <div className="mt-0.5 space-y-0.5 overflow-hidden">
                ...
                {events.length > 2 && (
                  <p className="text-[9px] text-slate-400 px-1">+{events.length - 2} más</p>
                )}
              </div>
            </div>
          );
          ...
          <div className="flex items-center gap-2">
            <div className="flex bg-slate-100 rounded-lg p-0.5">
              <button onClick={() => setView('list')}
                className={`p-1.5 rounded-md transition-colors ${view === 'list' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}>
                <List size={15} />
              </button>
              <button onClick={() => setView('calendar')}
                className={`p-1.5 rounded-md transition-colors ${view === 'calendar' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}>
                <Calendar size={15} />
              </button>
            </div>
            {isAdmin && (
        ...
        {isLoading ? <div className="flex justify-center py-16"><Spinner size={32} /></div> : (
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="ml-auto text-xs text-slate-400">{filtered.length} capacitaciones</span>
            </div>
              ...
              <div className="space-y-2">
                  ...
                  return (
                    <div key={t.id} onClick={() => setSelected(t.id)}
                      className={`bg-white rounded-xl border p-4 cursor-pointer hover:shadow-md transition-all
                        ${isPast ? 'border-amber-200 bg-amber-50' : 'border-slate-200 hover:border-blue-200'}`}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-extrabold text-slate-900 leading-snug">{t.title}</p>
                          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                            <span className="flex items-center gap-1 text-[11px] text-slate-500">
                              <Calendar size={11} />
                              {new Date(t.scheduled_date).toLocaleDateString('es-AR', { weekday:'short', day:'numeric', month:'short' })}
                            </span>
                            {t.location && <span className="text-[11px] text-slate-400">📍 {t.location}</span>}
                            {t.instructor && <span className="text-[11px] text-slate-400">👤 {t.instructor}</span>}
                            {t.duration_hours && <span className="text-[11px] text-slate-400">⏱ {t.duration_hours}hs</span>}
                          </div>
                        </div>
                ...
                {filtered.length === 0 && (
                  <div className="text-center py-16 text-slate-400">
                    ...
                    <p className="font-semibold text-slate-500">Sin capacitaciones</p>
                  </div>
                )}
              </div>
            )}
