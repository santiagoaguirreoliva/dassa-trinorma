      ...
      <PageContent>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
            <button onClick={() => setView('calendar')} className={`px-3 py-1.5 rounded-md text-[13px] font-semibold ${view === 'calendar' ? 'bg-white text-dassa-red-deep shadow-sm' : 'text-gray-500'}`}>
              Calendario
            </button>
            <button onClick={() => setView('list')} className={`px-3 py-1.5 rounded-md text-[13px] font-semibold ${view === 'list' ? 'bg-white text-dassa-red-deep shadow-sm' : 'text-gray-500'}`}>
              Lista
            </button>
          </div>
          {view === 'calendar' && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-gray-700 min-w-[140px] text-center">{MONTHS[month]} {year}</span>
              <button onClick={nextMonth} className="p-1.5 hover:bg-gray-100 rounded-lg"><ChevronRight size={16} /></button>
              <button onClick={goToday} className="ml-2 px-2.5 py-1 text-[12px] font-semibold text-dassa-red hover:bg-dassa-red-tint rounded-lg">Hoy</button>
            </div>
          )}
        </div>
        ...
        {view === 'calendar' ? (
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-xl overflow-hidden border border-gray-200">
                {/* Header */}
                {DAYS.map((d) => (
                  <div key={d} className="bg-gray-50 text-center py-2 text-[11px] font-bold text-gray-500 uppercase">{d}</div>
                ))}
                {/* Blanks */}
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white min-h-[80px] p-1.5 cursor-pointer hover:bg-dassa-red-tint/50 transition-colors ${isSelected ? 'ring-2 ring-dassa-red ring-inset' : ''}`}
                    >
                      <span className={`text-[12px] font-semibold inline-flex items-center justify-center w-6 h-6 rounded-full ${isToday ? 'bg-dassa-red text-white' : 'text-gray-600'}`}>
                        {day}
                      </span>
            ...
            <div className="w-72 flex-shrink-0">
              <div className="bg-white border border-gray-200 rounded-xl p-4 sticky top-4">
                ...
                {!selectedDate && <p className="text-[12px] text-gray-400">Hacé clic en un día del calendario para ver sus eventos.</p>}
                {selectedDate && selectedEvents.length === 0 && <p className="text-[12px] text-gray-400">Sin eventos este día.</p>}
          ...
          <div className="space-y-2">
            {allEvents.length === 0 && <p className="text-center text-gray-400 text-[13px] py-10">No hay eventos en los próximos 60 días.</p>}
            {allEvents.map((ev, i) => {
              const cfg = EVENT_CONFIG[ev.type];
              const Icon = cfg.icon;
              return (
                <div key={i} className="bg-white border border-gray-200 rounded-lg p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold text-gray-700 truncate">{ev.label}</p>
