...
function RiskMatrix({ risks }: { risks: Risk[] }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <p className="text-[13px] font-bold text-gray-800 mb-1">Matriz de Riesgos (Probabilidad × Severidad)</p>
      <p className="text-[11px] text-gray-400 mb-4">Cada celda muestra la cantidad de riesgos en esa intersección</p>
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead>
            <tr>
              <th className="w-20 p-2 text-[10px] text-gray-400 font-bold">P \ S</th>
              {[1,2,3,4,5].map(s => (
                <th key={s} className="p-2 text-[10px] text-gray-500 font-bold text-center w-20">
                  Sev. {s}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {[5,4,3,2,1].map(p => (
              <tr key={p}>
                <td className="p-2 text-[10px] text-gray-500 font-bold">Prob. {p}</td>
                {[1,2,3,4,5].map(s => {
                  const ir = p * s;
                  const key = `${p}-${s}`;
                    ...
                    <td key={s} className={`p-1.5 text-center border ${border} ${bg} rounded`}>
                      {count > 0 ? (
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-[8px] text-gray-400">IR={ir}</span>
                        </div>
                      ) : (
                        <span className="text-[9px] text-gray-300">{ir}</span>
                      )}
                    </td>
                  );
                })}
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-[15px] font-extrabold text-gray-900">{isEdit ? 'Editar Riesgo' : 'Nuevo Riesgo'}</h3>
        ...
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="bg-gray-50 rounded-lg p-3 flex items-center justify-between">
            <span className="text-xs font-bold text-gray-500">Índice de Riesgo (P × S)</span>
            <IRBadge ir={ir} />
          </div>
          <div>
      ...
      <PageContent>
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-[13px] font-bold text-gray-800 mb-1">Distribución por Nivel de Riesgo</p>
              <p className="text-[11px] text-gray-400 mb-4">Cantidad de riesgos por categoría de IR</p>
